...
const headCells = [
  { id: 'name', numeric: false, disablePadding: true, label: '產品名稱' },
...
export default function EnhancedTable() {
  ...
  useEffect(()=>{
    ...
    base('sale').select({
        ...
        records.forEach(function(record) {
            product_name.push(record.fields.product_name[0]);
        ...
        getperformance(product_name,prices,sale_count,buy_shop);
    });

    },[]);

    const [sale_performance, setPerformance] = useState([]);
    function getperformance(name,prices,sale_count){
      console.log(prices);
        var arr = [];
        const product_name = Array.from(new Set(name.map((record)=>record)));//取出buy_records裡的員工姓名，且用Set移除重複的名字
        console.log(product_name);
        for(var i of product_name){
            console.log(i);
            var price = 0;
            var count = 0;
            var countnum = -1;
  ...
  const emptyRows = rowsPerPage - Math.min(rowsPerPage, sale_performance.length - page * rowsPerPage);
    ...
    function Taipei(){
      ...
      base('sale').select({
          ...
          console.log(sale_count);
            getperformance(product_name,prices,sale_count);
    ...
    function Taichung(){
      ...
      base('sale').select({
          ...
          console.log(prices);
          console.log(sale_count);
            getperformance(product_name,prices,sale_count,store);
  ...
  return (
    <div className={classes.root}>
        <div>
            <p></p>
            <Link to="/home/Name"><Button width="25px" variant="contained" color="inherit" >名稱</Button></Link>
