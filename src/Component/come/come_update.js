...
export default function Come_update(props) {
    ...
    const [value_prod, setValue_prod] = useState(
        !come.fields.com_product_name ? '無' : come.fields.com_product_name.toString(),
    )
    //得知管道選單
                ...
                <DialogContent>
                    ...
                    <Select label="得知管道" fullWidth value={newKnow} onChange={ChangeKnow} MenuProps={MenuProps}>
                        {infomenu.map((infomenu) => (
                            <MenuItem key={infomenu} value={infomenu}>
                                {infomenu}
                            </MenuItem>
                        ))}
                    </Select>
                    <Autocomplete
                        ...
                        renderInput={(params) => <TextField {...params} label="有興趣的產品" margin="normal" />}
                    />
                    <TextField
                        margin="dense"
                        label="停留時長"
