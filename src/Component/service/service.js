...
export default function Service_search(props){
    ...
    function filter_ser(inputvalue){
        ...
        var filter_temp = service.filter((this_service) => {
            ...
            return ser_cus_name.toLowerCase().includes(inputvalue.toLowerCase())
    ...
    return(
        <div>
          <Grid container direction="row" justify="space-between" alignItems="center" spacing={3}>
            <Grid item>
              <input type="text" id="search" placeholder="搜尋" onChange={e => filter_ser(e.target.value)} style={{ color: "black" }} />
            </Grid>
            <Grid item>
              <Grid container direction="row" justify="flex-end" alignItems="center" spacing={3}>
                <Grid item>
                    {SelectedId_arr.length === 1 && <ServiceUpdate update_id={SelectedId_arr[0]} service={SelectedService} />}
                </Grid>
                <Grid item>
                    {SelectedId_arr.length > 0 && <ServiceDelete delete_id={SelectedId_arr} />}
                </Grid>
                <Grid item><ServiceCreate /></Grid>
              </Grid>
            </Grid>
          </Grid>
                    ...
                    <Table tablename="service">
                        ...
                        <TableBody>
                            {(rowsPerPage > 0
                                ...
                                : filteredService).map((this_service)=>(
                                <TableRow key={this_service.fields.ser_id}>
                                    <TableCell padding="checkbox">
                                        <Checkbox id={this_service.fields.ser_id.toString()} onClick={handleSelect}/>
                                    </TableCell>
                                    <TableCell>{this_service.fields.ser_cus_name}</TableCell>
                                    <TableCell>{this_service.fields.ser_productname}</TableCell>
                                    <TableCell>{moment(this_service.fields.ser_actualdate).format('YYYY-MM-DD HH:mm')}</TableCell>
