...
const headCells = [
  { id: 'name', numeric: false, disablePadding: true, label: '產品尺寸' },
  ...
  useEffect(()=>{
    ...
    base('sale').select({
        ...
        records.forEach(function(record) {
            product_fixedsize.push(record.fields.product_fixedsize[0]);
        ...
        getperformance(product_fixedsize,prices,sale_count,buy_shop);
    });

    },[]);

    const [sale_performance, setPerformance] = useState([]);
    function getperformance(fixedsize,prices,sale_count){
      console.log(prices);
        var arr = [];
        const product_fixedsize = Array.from(new Set(fixedsize.map((record)=>record)));//取出buy_records裡的員工姓名，且用Set移除重複的名字
        console.log(product_fixedsize);
        for(var i of product_fixedsize){
            console.log(i);
            var price = 0;
            var count = 0;
            var countnum = -1;
            for(var j of fixedsize){
                console.log(j);
                if(i === j){
                    countnum += 1;
  ...
  const emptyRows = rowsPerPage - Math.min(rowsPerPage, sale_performance.length - page * rowsPerPage);
    ...
    function Taipei(){
      ...
      base('sale').select({
          ...
          console.log(sale_count);
            getperformance(product_fixedsize,prices,sale_count);
    ...
    function Taichung(){
      ...
      base('sale').select({
          ...
          console.log(prices);
          console.log(sale_count);
            getperformance(product_fixedsize,prices,sale_count,store);
  ...
  return (
    <div className={classes.root}>
        ...
        <p></p>
            ...
            <Link to="/home/fixedsize"><Button width="25px" variant="contained" color="inherit" >尺寸</Button></Link>
            <Link to="/home/producers"><Button width="25px" variant="contained" color="default" >製造商</Button></Link>
