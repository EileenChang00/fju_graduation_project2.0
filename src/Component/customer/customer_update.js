...
export default function Customer_update(props){
    ...
    function create(introSuccess){
        ...
        if(!newBirth){
            if(!newminorintro){
                var customer_record = {"id": props.update_id,
                "fields": {
            ...
            "cus_invoice": newinvoice
                ...
                customer_record = {"id": props.update_id,
                "fields": {
            "cus_name": newCustomerName,
            "cus_phone": newPhone,
            "cus_profession": newProfession,
        ...
        introSuccess && base('customer').update([customer_record], function(err, records) {
        if (err) {
            console.error(err);
            alert("輸入資料有誤，請確認");
            return;
    ...
    function find_oldintro(introSuccess){
        customer.fields.customer ? base('customer').find(customer.fields.customer, function(err, record) {
            if (err) { alert("介紹人(顧客)資料新增失敗"); return; }
        ...
        <div>
            ...
            <Dialog open={open} onClose={handleClose}>
                ...
                <DialogContent>
                    ...
                    <TextField margin="dense" label="手機" type="text" value={newPhone} onChange={(event) => phoneFormat(event.target.value)} fullWidth error={phoneError} helperText={phoneError && "輸入長度有誤"}/>
                    ...
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                        <TextField {...params} label="介紹人(顧客)" margin="normal" />
                        )}
                    />
                    <Autocomplete
                        freeSolo
                        ...
                        getOptionLabel={(option) => option.name}
                        renderInput={(params) => (
                        <TextField {...params} label="介紹人(設計師)" margin="normal" />
                        )}
                    />
