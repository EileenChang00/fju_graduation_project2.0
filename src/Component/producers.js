...
const headCells = [
  { id: 'name', numeric: false, disablePadding: true, label: '產品製造商' },
...
export default function EnhancedTable() {
  ...
  useEffect(()=>{
    ...
    base('sale').select({
        ...
        records.forEach(function(record) {
            product_producer_name.push(record.fields.product_producer_name[0]);
        ...
        getperformance(product_producer_name,prices,sale_count,buy_shop);
    });

    },[]);

    const [sale_performance, setPerformance] = useState([]);
    function getperformance(producer,prices,sale_count){
      console.log(prices);
        var arr = [];
        const product_producer_name = Array.from(new Set(producer.map((record)=>record)));//取出buy_records裡的員工姓名，且用Set移除重複的名字
        console.log(product_producer_name);
        for(var i of product_producer_name){
            console.log(i);
            var price = 0;
            var count = 0;
            var countnum = -1;
            for(var j of producer){
                console.log(j);
                if(i === j){
                    countnum += 1;
  ...
  const emptyRows = rowsPerPage - Math.min(rowsPerPage, sale_performance.length - page * rowsPerPage);
    ...
    function Taipei(){
      ...
      base('sale').select({
          ...
          console.log(sale_count);
            getperformance(product_producer_name,prices,sale_count);
    ...
    function Taichung(){
      ...
      base('sale').select({
          ...
          console.log(prices);
          console.log(sale_count);
            getperformance(product_producer_name,prices,sale_count,store);
  ...
  return (
    <div className={classes.root}>
        <div>
        <p></p>
            <Link to="/home/Name"><Button width="25px" variant="contained" color="default" >名稱</Button></Link>
            <Link to="/home/category"><Button width="25px" variant="contained" color="default" >類別</Button></Link>
            <Link to="/home/series"><Button width="25px" variant="contained" color="default" >系列</Button></Link>
            <Link to="/home/fixedsize"><Button width="25px" variant="contained" color="default" >尺寸</Button></Link>
            <Link to="/home/producers"><Button width="25px" variant="contained" color="inherit" >製造商</Button></Link>
