...
export default function Buy_update(props) {
    ...
    useEffect(() => {
        buy.fields.sale_id &&
            buy.fields.sale_id.forEach(function (sale_id) {
                base('sale').find(sale_id, function (err, record) {
                    ...
                    buy_sale.push([record.fields.product_name[0], record.fields.sale_count, record.fields.product[0]])
                })
            })
    }, [])
    ...
    const addSale = () => {
        value_sale.includes(saleName)
            ...
            : value_sale.push([saleName, saleCount, saleId])
    ...
    var buy_gift = []
    useEffect(() => {
        buy.fields.buy_gift_id &&
            buy.fields.buy_gift_id.forEach(function (gift_id) {
                base('gift').find(gift_id, function (err, record) {
                    ...
                    console.log('Retrieved', record.id)
                    buy_gift.push([record.fields.product_name[0], record.fields.gift_count, record.fields.product[0]])
                })
            })
    }, [])
    ...
    const addGift = () => {
        value_gift.includes(giftName)
            ...
            : value_gift.push([giftName, giftCount, giftId])
                        ...
                        <form onSubmit={handleSubmitCustomer}>
                            ...
                            <DialogContent>
                                ...
                                <TextField
                                    ...
                                    onChange={(event) =>
                                        setDialogValue_cus({ ...dialogValue_cus, phone: event.target.value })
                                    }
                                    label="手機"
                                    type="number"
                                />
