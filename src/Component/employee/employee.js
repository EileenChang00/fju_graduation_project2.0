...
export default function Employee(props) {
    ...
    function filter_em(inputvalue) {
        ...
        var filter_temp = employee.filter((this_employee) => {
            ...
            return this_employee.fields.em_name.toLowerCase().includes(inputvalue.toLowerCase())
    ...
    return (
        <div>
          <Grid container direction="row" justify="space-between" alignItems="center" spacing={3}>
            <Grid item>
              <input type="text" id="search" placeholder="搜尋" onChange={e => filter_em(e.target.value)} style={{ color: "black" }} />
            </Grid>
            <Grid item>
              <Grid container direction="row" justify="flex-end" alignItems="center" spacing={3}>
                <Grid item>
                    {SelectedId_arr.length === 1 && <EmployeeUpdate update_id={SelectedId_arr[0]} employee={SelectedEmployee} />}
                </Grid>
                <Grid item>
                    {SelectedId_arr.length > 0 && <EmployeeDelete delete_id={SelectedId_arr} />}
                </Grid>
                    ...
                    <Table tablename='employee' className={classes.table}>
                        ...
                        <TableBody>
                            {(rowsPerPage > 0
                                ...
                                : filteredEmployee).map((this_employee)=>(
                                    <TableRow key={this_employee.fields.em_id}>
                                        <TableCell padding="checkbox">
                                            <Checkbox id={this_employee.fields.em_id.toString()} onClick={handleSelect}/>
                                        </TableCell>
                                        ...
                                        <TableCell>{!this_employee.fields.em_resigndate ? '無' : this_employee.fields.em_resigndate}</TableCell>
                                    </TableRow>
                                ))}
                                {emptyRows > 0 && (
